...
public class LogAdapter extends RecyclerView.Adapter<LogAdapter.MyViewHolder> {
    ...
    public void getNewData(){
        List<LogLine> dts = ObjectBox.get().boxFor(LogLine.class).query().greater(LogLine_.time, bigTime).order(LogLine_.time).build().find(offset,limit);
    ...
    public void nextData(){
        List<LogLine>  dts = ObjectBox.get().boxFor(LogLine.class).query().less(LogLine_.time, smallTime).orderDesc(LogLine_.time).build().find(offset,limit);
        for(int n=0;n<dts.size();n++){
            datas.add(dts.get(n));
